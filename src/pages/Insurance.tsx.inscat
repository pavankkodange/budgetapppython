...
import {
  Shield, Plus, FileText, Heart, Car, Home, Edit, Trash2, Calendar, DollarSign, Info, Plane, AlertTriangle, Bell
} from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
...
const Insurance = () => {
  ...
  const [selectedPolicyForDocs, setSelectedPolicyForDocs] = useState<InsurancePolicy | null>(null);
    ...
    <>
      <header className="p-3 sm:p-4 border-b border-border flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center space-x-2">
          <BackButton />
          <div>
            <h1 className="text-xl sm:text-2xl font-bold">Insurance</h1>
            <p className="text-xs sm:text-sm text-muted-foreground hidden sm:block">Manage your insurance policies and coverage</p>
          </div>
        </div>
          ...
          <Dialog open={isPolicyDialogOpen} onOpenChange={setIsPolicyDialogOpen}>
            <DialogTrigger asChild>
              <Button size="sm" className="h-9 sm:h-10 shrink-0">
                <Plus className="h-4 w-4 mr-1 sm:mr-2" /> Add Policy
              </Button>
            </DialogTrigger>
      ...
      <main className="flex-1 p-3 sm:p-6 overflow-auto">
        <Tabs defaultValue="overview" className="space-y-4 sm:space-y-6">
          <TabsList className="flex w-full overflow-x-auto scrollbar-none bg-muted/50 p-1 h-auto min-h-10">
            ...
            <TabsTrigger value="term-life" className="flex-1 min-w-[100px] text-xs sm:text-sm">Term Life</TabsTrigger>
            <TabsTrigger value="health" className="flex-1 min-w-[100px] text-xs sm:text-sm">Health</TabsTrigger>
            <TabsTrigger value="other" className="flex-1 min-w-[110px] text-xs sm:text-sm">Other</TabsTrigger>
            <TabsTrigger value="documents" className="flex-1 min-w-[100px] text-xs sm:text-sm">Docs</TabsTrigger>
          </TabsList>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <DocumentUpload
                        ...
                        onUpload={(document) => uploadDocument(selectedPolicyForDocs.id, { ...document, documentType: document.documentType as any })}
