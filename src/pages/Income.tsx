...
import { Copy, Save, Plus, Trash2, ArrowLeft } from "lucide-react";
import { showInfo, showError, showSuccess, showLoading, dismissToast } from "@/utils/toast";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
const IncomePage = () => {
  ...
  const { saveMonthlyIncomeSummary, getMonthlyIncomeSummary } = useIncomeSummaries();
  ...
  useEffect(() => {
    const summaryForSelectedPeriod = getMonthlyIncomeSummary(parseInt(selectedMonth), parseInt(selectedYear));
  ...
  const handleCopyFromPreviousMonth = () => {
    ...
    const previousSummary = getMonthlyIncomeSummary(prevMonth, prevYear);

    if (previousSummary) {
      // Convert numbers back to strings for display in inputs
      const stringifiedLineItems: { [key: string]: string } = {};
      for (const key in previousSummary.lineItems) {
        stringifiedLineItems[key] = previousSummary.lineItems[key].toString();
      }
      setLineItemAmounts(stringifiedLineItems);
      
      // Copy additional incomes if they exist
      if (previousSummary.additionalIncomes) {
        setAdditionalIncomes(previousSummary.additionalIncomes);
      }
      
      setHasUserEdited(true); // Mark as edited so it can be saved
      showInfo(`Copied income data from ${months[prevMonth].label} ${prevYear}.`);
    } else {
      showError(`No income data found for ${months[prevMonth].label} ${prevYear} to copy from.`);
    }
  };

  const handleCopyToFutureMonths = () => {
    const currentMonthIndex = parseInt(selectedMonth);
    const currentYearValue = parseInt(selectedYear);
    
    // Convert current line item amounts to numbers for saving
    const numericCurrentLineItems: { [key: string]: number } = {};
    for (const key in lineItemAmounts) {
      numericCurrentLineItems[key] = parseFloat(lineItemAmounts[key]) || 0;
    }

    let copiedCount = 0;
    for (let m = currentMonthIndex + 1; m < 12; m++) {
      const futureMonth = m;
      const futureYear = currentYearValue;

      const newSummaryData: Omit<MonthlyIncomeSummary, 'id'> = {
        month: futureMonth,
        year: futureYear,
        lineItems: numericCurrentLineItems, // Use numeric values for saving
        additionalIncomes: additionalIncomes, // Copy additional incomes too
      };
      saveMonthlyIncomeSummary(newSummaryData);
      copiedCount++;
    }
    ...
    if (copiedCount > 0) {
      showSuccess(`Copied income data to ${copiedCount} future months in ${currentYearValue}.`);
    } else {
      showInfo("No future months to copy to in the selected year.");
    }
  };
        ...
        <header className="p-3 sm:p-4 border-b border-border flex items-center justify-between">
          ...
          <div className="flex items-center space-x-1 sm:space-x-2">
            <Select onValueChange={setSelectedMonth} defaultValue={selectedMonth} >
              <SelectTrigger className="w-[90px] sm:w-[140px] text-xs sm:text-sm h-8 sm:h-10">
            ...
            <Button onClick={handleCopyFromPreviousMonth} variant="outline" size={isMobile ? "sm" : "default"} className={isMobile ? "hidden sm:flex" : ""}>
              <Copy className="h-4 w-4 mr-2" /> {isMobile ? "Copy" : "Copy from Previous"}
            </Button>
            <AlertDialog>
              <AlertDialogTrigger asChild className={isMobile ? "hidden sm:flex" : ""}>
                <Button variant="outline" size={isMobile ? "sm" : "default"}>
                  <Copy className="h-4 w-4 mr-2" /> {isMobile ? "Copy" : "Copy to Future"}
                </Button>
              </AlertDialogTrigger>
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>Copy to Future Months?</AlertDialogTitle>
                  <AlertDialogDescription>
                    This will copy the current month's income and deduction data to all subsequent months in {selectedYear}.
                    This action will overwrite any existing data for those months. Are you sure you want to proceed?
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                  <AlertDialogCancel>Cancel</AlertDialogCancel>
                  <AlertDialogAction onClick={handleCopyToFutureMonths}>
                    Continue
                  </AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
            </AlertDialog>
            <Button onClick={handleSaveMonthlyIncome} disabled={!hasUserEdited || isSaving} size={isMobile ? "sm" : "default"}>
            ...
            <TabsContent value="regular-income">
              <div className="bg-card p-3 sm:p-6 rounded-lg shadow-sm border border-border mb-4 sm:mb-8">
                <h2 className="text-lg sm:text-xl font-semibold mb-3 sm:mb-4 flex items-center justify-between">
                  Income for {months[parseInt(selectedMonth)].label} {selectedYear}
                </h2>
              ...
              <div className="bg-card p-3 sm:p-6 rounded-lg shadow-sm border border-border mb-4 sm:mb-8">
                <div className="flex justify-between items-center mb-3 sm:mb-4">
                  <h2 className="text-lg sm:text-xl font-semibold">
                    Additional Income for {months[parseInt(selectedMonth)].label} {selectedYear}
                  </h2>
